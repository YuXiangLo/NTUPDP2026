...
def carve_seams_torch(
    ...
    if device is None:
        device = image.device
    carved = image.to(device=device, dtype=torch.float32)
    ...
    if show_progress:
        ...
        indices = tqdm(indices, desc=f"Carving (Torch {device.type})", leave=False)
...
def _compute_energy(image: torch.Tensor) -> torch.Tensor:
...
def _find_vertical_seam(energy: torch.Tensor) -> torch.Tensor:
...
def _remove_vertical_seam(image: torch.Tensor, seam: torch.Tensor) -> torch.Tensor:
