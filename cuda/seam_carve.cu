// seam_carve.cu - Standalone CUDA vertical seam carving.
//
//   ./seam_carve <input.jpg> <num_seams> [output.png]
//
// The whole pipeline runs on the GPU: grayscale, energy (gradient magnitude),
// the cumulative-cost dynamic program, and seam removal are each custom
// kernels. No PyTorch / Python involved. Image I/O uses stb_image (host side).
//
...
// Fused whole-image DP: one block walks rows top-to-bottom, prev/curr cost rows
// in shared memory (ping-pong). argmin of the last row + backtrack done by
// thread 0. One launch per seam instead of ~H tiny launches.
